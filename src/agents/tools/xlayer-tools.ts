...
async function inspectAddress(argumentsValue: unknown): Promise<AgentToolResult> {
  ...
  try {
    ...
    const mainnetData = {
      ...
      chainId: 196,
      address: input.address,
      nativeBalance: (mainnetSnapshot.balance as string) ?? "0",
      nativeSymbol: (mainnetSnapshot.balanceSymbol as string) ?? "OKB",
      transactionCount: (mainnetSnapshot.transactionCount as number) ?? 0,
      isContract: Boolean(mainnetSnapshot.contractAddress),
